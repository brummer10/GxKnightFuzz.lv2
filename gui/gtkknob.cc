...
#define GTK_KNIGHTFUZZ_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_KNIGHTFUZZ_KNOB, GtkKnightFuzzKnobPrivate))

G_DEFINE_TYPE(GtkKnightFuzzKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkKnightFuzzKnobPrivate GtkKnightFuzzKnobPrivate;

struct _GtkKnightFuzzKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkKnightFuzzKnobPrivate *priv = GTK_KNIGHTFUZZ_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkKnightFuzzKnob *knob = GTK_KNIGHTFUZZ_KNOB(widget);
...
{
    g_assert(GTK_IS_KNIGHTFUZZ_KNOB(widget));
    GtkKnightFuzzKnob *knob = GTK_KNIGHTFUZZ_KNOB(widget);
    GtkKnightFuzzKnobPrivate *priv = GTK_KNIGHTFUZZ_KNOB_GET_PRIVATE(knob);
...
{
    GtkKnightFuzzKnob *knob = GTK_KNIGHTFUZZ_KNOB(object);
...
/****************************************************************
 ** init the GtkKnightFuzzKnobClass
 */

static void gtk_knob_class_init (GtkKnightFuzzKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkKnightFuzzKnobPrivate));
}
...
static void gtk_knob_init (GtkKnightFuzzKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_KNIGHTFUZZ_KNOB, NULL ));
