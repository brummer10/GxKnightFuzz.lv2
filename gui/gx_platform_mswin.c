...
void gx_gui_create_window_and_surface(gx_KnightFuzzUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_KnightFuzzUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_KnightFuzzUI *ui) {
	...
	UnregisterClass(TEXT("gx_KnightFuzzUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_KnightFuzzUI *ui = (gx_KnightFuzzUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
